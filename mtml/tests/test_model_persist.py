...
from ..utils.persist import persist_pickle
from ..utils.persist import persist_json
...
def test_persist_json():
    ...
    with open(abspath + "/.dtc_params.json", "r") as jf:
        dtc_params = json.load(jf)
    res = train_dtc(X_train, X_test, y_train, y_test, random_state = 7)
    assert acc == res[1]